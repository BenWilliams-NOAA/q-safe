...
Note if there was a transition to new software (e.g. ADMB to TMB or another modeling framework), provide a link to that analysis, which should present side-by-side comparisons of both data likelihood components and model parameter estimates in the results, and the percent differences.
Document data weighting methods used in the assessment.  -->

<!-- Update models:

Include a description of the base model included in the assessment, including a reference to the last full operational assessment when the base model was used for management advice.
